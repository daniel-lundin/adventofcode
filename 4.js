...
function anagram(a, b) {
  ...
  return Array.from(a).reduce(({ valid, otherWord }, letter) => {
    return {
      valid: valid && otherWord.includes(letter),
      otherWord: otherWord.replace(letter, '')
    }
    valid && letter
  }, { valid: true, otherWord: b }).valid;
...
function isValid2(string) {
  ...
  for (let i=0; i<parts.length - 1; ++i) {
    const word = parts[i];
    const restWords = parts.slice(i + 1);
    for(j in restWords) {
      const otherWord = restWords[j];
      if (anagram(word, otherWord)) {
        return false;
      }
    }
  }
...
const valids = input.reduce((acc, part) => {
  if (isValid(part)) return acc + 1;
  return acc;
}, 0);

const valids2 = input.reduce((acc, part) => {
  if (isValid2(part)) return acc + 1;
  return acc;
}, 0);

console.log('answer part 1 is', valids);
console.log('answer part 2 is', valids2);
